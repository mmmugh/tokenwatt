...
def _spans_multiple_durations(samples: list[dict]) -> bool:
    """True if any cell name spans a large MEASURED-Δt ratio — the fingerprint of a
    hand-merged multi-duration campaign. The threshold is on measured, not nominal, dt:
    run_cell's dt_s overshoots the target cell_seconds by up to one request latency, and
    the SHORTER cell overshoots proportionally more, so the sweep's tightest genuine merge
    (360s+720s) measures only ~1.7× — not the nominal 2×. Meanwhile a single-duration
    cell's passes jitter at most ~1.5× (well-sized cells stay near 1.1×). A >1.6× within-
    cell ratio therefore sits in the gap: it flags the merges that would inflate `fit()`'s
    by-cell band (redirecting to `fit_combined`) while clearing measurement jitter on valid
    data. Best-effort: a near-equal merge (<1.6× measured) is let through, and fit() then
    reports a wide, HONEST band for it — it never fabricates a number."""
    by_cell: dict[str, list[float]] = {}
    for s in samples:
        by_cell.setdefault(s["cell"], []).append(s["dt_s"])
    return any(min(dts) > 0 and max(dts) / min(dts) > 1.6 for dts in by_cell.values())
