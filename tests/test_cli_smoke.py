...
def test_calibrate_fit_writes_a_profile_from_a_campaign(tmp_path, monkeypatch):
    ...
    # deterministic machine identity, independent of the CI runner. Patching
    # machineid._real_sysctl is INERT (it's a keyword-default bound at def time); patch
    # detect_machine itself — the command re-fetches it via a local import at call time.
